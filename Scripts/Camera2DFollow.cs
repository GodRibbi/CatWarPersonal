...
namespace UnityStandardAssets._2D
{
    public class Camera2DFollow : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            {
                m_LookAheadPos = lookAheadFactor * 
                    (Vector3.up * Mathf.Sign(yMoveDelta)+ Vector3.right * Mathf.Sign(xMoveDelta));
            }
            else if (updateLookAheadTargety)
            {
                m_LookAheadPos = lookAheadFactor * Vector3.up * Mathf.Sign(yMoveDelta);
            }
            else if (updateLookAheadTargetx)
            {
